...
const GlobalStyle = ({ dark }) => (
  <style>{`
    ...
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .project-item { margin-bottom: .95rem; position: relative; }
    .project-name { font-weight: 600; font-size: .81rem; color: #1a1a2e; }
    .project-tech { font-size: .72rem; color: #777; font-style: italic; margin-bottom: .25rem; }
    .project-desc { font-size: .75rem; color: #555; line-height: 1.55; }
    .project-bullets { list-style: none; margin-top: .2rem; }
    .project-bullets li { font-size: .76rem; color: #444; line-height: 1.6; padding-left: .9rem; position: relative; margin-bottom: .17rem; }
    .project-bullets li::before { content: '▸'; position: absolute; left: 0; font-size: .52rem; top: .19rem; }
...
function ResumeContent({ accent, rSummary, exp, projects, education, certifications, languages, awards, skills,
  ...
  return <>
    <div className="r-section">
      ...
      <div className="r-summary" {...CE}>{rSummary}</div>
    </div>
      ...
      <div className="r-section">
        ...
        {projects.map(p => (
          <div className="project-item" key={p.id}>
            ...
            {p.tech && <div className="project-tech" {...CE}>Tech: {p.tech}</div>}
            {p.description && <div className="project-desc" {...CE}>{p.description}</div>}
            {p.bullets && p.bullets.length > 0 && (
              <ul className="project-bullets">
                {p.bullets.map((b, j) => <li key={j}><span {...CE}>{b}</span></li>)}
              </ul>
            )}
          </div>
        ))}
...
function PaginatedResume({ accent, font, fontSize,
  rName, rTitle, rEmail, rPhone, rLocation, rLinkedin, rSummary,
  exp, projects, education, certifications,
  ...
  const sharedContentProps = { accent, rSummary, exp, projects, education, certifications, languages, awards, skills, delExp, delProject, delEdu, delSkill, delCert, delLang, delAward };
    ...
    <div className="resume-tray" ref={containerRef}>
      <div ref={fullRef} style={{ position: "absolute", visibility: "hidden", pointerEvents: "none", width: containerW || "100%", fontFamily: font, fontSize: fzMap[fontSize], left: 0, top: 0, zIndex: -1 }}>
        <div className="r-header" style={{ background: accent.header }}>
          <div className="r-name">{rName}</div>
          <div className="r-title">{rTitle}</div>
        </div>
              ...
              <>
                <div className="r-header" style={{ background: accent.header }}>
                  <div className="r-name">
                    <span contentEditable suppressContentEditableWarning>{rName}</span>
                  </div>
                  <div className="r-title">
                    <span contentEditable suppressContentEditableWarning>{rTitle}</span>
                  </div>
                  <div className="r-contact">
                    {[{ icon: "✉", val: rEmail }, { icon: "☎", val: rPhone }, { icon: "⌖", val: rLocation }, { icon: "in", val: rLinkedin }].map((c, i) => (
...
function DownloadModal({ accent, font, fontSize,
  rName, rTitle, rEmail, rPhone, rLocation, rLinkedin, rSummary,
  exp, projects, education, certifications, languages, awards, skills, onClose }) {
  const fzMap = { small: "0.74rem", medium: "0.80rem", large: "0.88rem" };
  const sharedContentProps = {
    accent, rSummary, exp, projects, education, certifications, languages, awards, skills,
    ...
    win.document.write(`<!DOCTYPE html><html><head>
      ...
      <style>
        *{box-sizing:border-box;margin:0;padding:0;} body{font-family:'Sora',sans-serif;background:#fff;color:#1a1a2e;}
        ...
        .exp-bullets li::before{content:'▸';position:absolute;left:0;font-size:.52rem;top:.19rem;}
        .project-item{margin-bottom:.95rem;} .project-name{font-weight:600;font-size:.81rem;} .project-tech{font-size:.72rem;color:#777;font-style:italic;margin-bottom:.25rem;} .project-desc{font-size:.75rem;color:#555;line-height:1.55;}
        .project-bullets{list-style:none;margin-top:.2rem;} .project-bullets li{font-size:.76rem;color:#444;line-height:1.6;padding-left:.9rem;position:relative;margin-bottom:.17rem;}
        .project-bullets li::before{content:'▸';position:absolute;left:0;font-size:.52rem;top:.19rem;}
...
// --- Helper to parse backend final_resume text into structured sections ---
function parseResumeText(text) {
  ...
  let headerDone = false;  // once we hit first section header, stop looking for name/contact
  ...
  for (const raw of lines) {
    ...
    if (isSectionHeader(line)) {
      flush();
      headerDone = true;
      if (/SUMMARY|OBJECTIVE|PROFILE/.test(upper)) section = "summary";
      else if (/EXPERIENCE|EMPLOYMENT/.test(upper)) section = "experience";
      else if (/PROJECT/.test(upper)) section = "projects";
      else if (/EDUCATION|ACADEMIC/.test(upper)) section = "education";
      else if (/SKILL|COMPETENC|TECHNOLOG/.test(upper)) section = "skills";
      else if (/CERTIFIC/.test(upper)) section = "certifications";
      else section = "";
      continue;
    }

    // ── Header block: name + contact (before first section header) ─
    if (!headerDone) {
      // Extract email
      const emailM = line.match(/[\w.+%-]+@[\w.-]+\.[a-z]{2,}/i);
      if (emailM && !result.email) { result.email = emailM[0]; continue; }

      // Extract phone
      const phoneM = line.match(/\+?[\d][\d\s\-(). ]{7,}/);
      if (phoneM && !result.phone) { result.phone = phoneM[0].trim(); }
      ...
      if (line.includes("|")) {
        line.split("|").forEach(part => {
          const p = part.trim();
          if (/[\w.+%-]+@[\w.-]+\.[a-z]{2,}/i.test(p) && !result.email)
            result.email = p.match(/[\w.+%-]+@[\w.-]+\.[a-z]{2,}/i)[0];
          else if (/\+?[\d][\d\s\-().]{6,}/.test(p) && !result.phone)
            result.phone = p;
          else if (/linkedin/i.test(p) && !result.linkedin)
            result.linkedin = p;
          else if (/github/i.test(p) && !result.location)
            result.location = p;
        });
        continue;
      }

      // First short non-contact line = name
      if (!result.name && line.length < 50 &&
        !emailM && !/\d{5,}/.test(line) && !/http/i.test(line)) {
        result.name = line;
        continue;
      }
      // Second short line could be title
      if (result.name && !result.title && line.length < 60 &&
        !emailM && !/\d{5,}/.test(line)) {
        result.title = line;
        continue;
      }
      continue;
    ...
    switch (section) {
      ...
      case "experience":
        if (line.startsWith("•") || line.startsWith("-") || line.startsWith("▸") || line.startsWith("*")) {
          if (currentExp) currentExp.bullets.push(line.replace(/^[•\-▸*]\s*/, ""));
          else { /* bullet before any job header — skip */ }
        } else if (/no professional experience/i.test(line) || /none/i.test(line) || /^\[.*none.*\]/i.test(line) || /do not fabricate/i.test(line)) {
          // LLM explicitly said no experience — do nothing
        } else if (!currentExp) {
          // "Company | Role" or "Role\nCompany" format
          if (line.includes("|")) {
            const parts = line.split("|").map(p => p.trim());
            currentExp = { id: Date.now() + Math.random(), company: parts[0], role: parts[1] || "", period: parts[2] || "", bullets: [] };
          } else {
            currentExp = { id: Date.now() + Math.random(), company: line, role: "", period: "", bullets: [] };
          }
        } else if (!currentExp.role) {
          currentExp.role = line;
        } else if (!currentExp.period && /\d{4}/.test(line)) {
          currentExp.period = line;
        }
        break;

      case "projects": {
        // Strip leading bullet from the line for analysis
        const isBullet = /^[•\-▸*]/.test(line);
        const isSubBullet = /^\s+[-▸•*]/.test(raw);  // indented = sub-bullet of current project
        const cleaned = line.replace(/^[•\-▸*]\s*/, "");

        if (isSubBullet && currentProj) {
          // Indented sub-bullet → add to current project's bullets
          const bullet = raw.trim().replace(/^[-▸•*]\s*/, "");
          if (bullet) {
            if (!currentProj.bullets) currentProj.bullets = [];
            currentProj.bullets.push(bullet);
          }
        } else if (/^Tech:/i.test(line) || /^Tech:/i.test(cleaned)) {
          // Standalone "Tech: ..." line
          if (currentProj) currentProj.tech = (cleaned.match(/Tech:\s*(.*)/i) || ["", cleaned])[1];
        } else if (isBullet && cleaned.includes(":")) {
          // "• ProjectName: description (Tech: X, Y)" → new project
          flush();
          const colonIdx = cleaned.indexOf(":");
          let projName = cleaned.slice(0, colonIdx).trim();
          let desc = cleaned.slice(colonIdx + 1).trim();
          // Extract inline (Tech: ...) from description
          let tech = "";
          const techMatch = desc.match(/\(Tech:\s*([^)]+)\)\s*$/i);
          if (techMatch) {
            tech = techMatch[1].trim();
            desc = desc.slice(0, desc.indexOf(techMatch[0])).trim();
          }
          if (projName && !/^(no |none)/i.test(projName)) {
            currentProj = { id: Date.now() + Math.random(), name: projName, description: desc, tech: tech, bullets: [] };
          }
        } else if (!isBullet) {
          // Non-bullet line → new project name (pipe or plain)
          flush();
          const pipeIdx = line.indexOf("|");
          const projName = pipeIdx > -1 ? line.slice(0, pipeIdx).trim() : line;
          const projTech = pipeIdx > -1 ? line.slice(pipeIdx + 1).trim() : "";
          if (projName && !/^(no |none)/i.test(projName)) {
            currentProj = { id: Date.now() + Math.random(), name: projName, description: "", tech: projTech, bullets: [] };
          }
        } else {
          // Top-level bullet but no colon → treat as new project with full text as name
          flush();
          if (cleaned && !/^(no |none)/i.test(cleaned)) {
            currentProj = { id: Date.now() + Math.random(), name: cleaned, description: "", tech: "", bullets: [] };
          }
        }
        break;
      }
      ...
      case "education": {
        // "Degree — Institution (Year)" or "Degree\nInstitution\nYear"
        const eduLine = line.replace(/^[•\-▸*]\s*/, "");
        if (eduLine.includes("—") || eduLine.includes("-")) {
          const parts = eduLine.split(/[—-]/).map(p => p.trim());
          // Extract year
          const yearM = eduLine.match(/\((\d{4}[^)]*)\)|(\d{4}\s*[–-]\s*\d{4})|(\d{4})/);
          result.education.push({
            id: Date.now() + Math.random(),
            degree: parts[0] || "",
            school: parts[1] || "",
            year: yearM ? yearM[0].replace(/[()]/g, "").trim() : "",
          });
        } else if (/cgpa|gpa|grade/i.test(eduLine)) {
          // append to last edu entry
          if (result.education.length > 0)
            result.education[result.education.length - 1].degree +=
              " | " + eduLine;
        } else if (result.education.length === 0) {
          result.education.push({ id: Date.now() + Math.random(), school: eduLine, degree: "", year: "" });
        } else {
          // extra line — append to last entry school field
          const last = result.education[result.education.length - 1];
          if (!last.school) last.school = eduLine;
        }
        break;
      }

      case "skills": {
        // Handle "Category: skill1, skill2" OR plain "skill1, skill2"
        const colonIdx = line.indexOf(":");
        let skillLine = line;
        if (colonIdx > -1) skillLine = line.slice(colonIdx + 1);
        skillLine.split(/[,;|•*]/).forEach(s => {
          const t = s.trim().replace(/^[•\-▸*]\s*/, "");
          if (t && t.length < 50) result.skills.push(t);
        });
        break;
      }
      ...
      case "certifications": {
        ...
        if (certLine && !/^(none|n\/a)/i.test(certLine))
          result.certifications.push({ id: Date.now() + Math.random(), text: certLine });
        break;
      }
...
export default function ResumePage({ onBack, dark, onToggleDark, resumeData }) {
  // Parse backend final_resume.txt into structured data
  const parsed = resumeData?.finalResume ? parseResumeText(resumeData.finalResume) : null;
  const initialData = parsed || defaultResume;
  const sessionId = resumeData?.sessionId || null;
  ...
  const copyPlain = useCallback(() => {
    ...
    lines.push(rTitle);
    lines.push(`${rEmail} | ${rPhone} | ${rLocation} | ${rLinkedin}`);
    lines.push("");
    lines.push("SUMMARY");
    lines.push(rSummary);
    lines.push("");
      ...
      <div className="app-shell">
        <nav className="nav">
          ...
          <div className="nav-right">
            <button className="theme-btn" onClick={onToggleDark} title="Toggle theme">{dark ? "☀️" : "🌙"}</button>
          </div>
        </nav>
            ...
            <div className="editor-panel">
              ...
              <button className="btn-download" onClick={() => {
                if (sessionId) {
                  ...
                  window.open(`${API_BASE}/api/download-pdf/${sessionId}`, "_blank");
                } else {
