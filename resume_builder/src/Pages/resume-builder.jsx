...
// ─────────────────────────────────────────────────────────────────────────────
// GlobalStyle – all CSS in one place
// ─────────────────────────────────────────────────────────────────────────────
const GlobalStyle = ({ dark }) => (
  <style>{`
    ...
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .page-wide   { max-width: 1200px; margin: 0 auto; padding: 0 1.5rem 4rem; }
    ...
    /* ── RESUME TRAY (dark inset behind pages) ── */
    ...
    /* First page has colored header, subsequent pages have a thin accent strip */
    .page-card-cont { padding: 1.8rem 2.2rem; }

    /* ── RESUME CONTENT (shared doc styles) ── */
    ...
    @media print {
      * { -webkit-print-color-adjust: exact !important; print-color-adjust: exact !important; }
      ...
      .nav, .stepper, .editor-panel, .page-wide > div:first-child, .page-badge, .tray-page-label, .resume-tray { display: none !important; }
...
function Stepper({ step }) {
  const steps = [{ id: "upload", label: "Upload" }, { id: "jobdesc", label: "Job Brief" }, { id: "resume", label: "Resume" }];
...
// ─── Shared resume section renderer ──────────────────────────────────────────
// Returns an array of JSX nodes that represent the resume body content
...
// ─── Paginated resume tray ────────────────────────────────────────────────────
// Renders the resume as actual stacked A4-page cards split at the page boundary.
function PaginatedResume({ accent, font, fontSize, exp, projects, education, certifications,
  ...
  const [pages, setPages] = useState([{ id: 1 }]); // at least 1 page

  const fzMap = { small: "0.74rem", medium: "0.80rem", large: "0.88rem" };

  // Measure single-page height → compute page breaks
  useEffect(() => {
    if (!fullRef.current || !containerRef.current) return;
    const containerW = containerRef.current.offsetWidth - 48; // minus tray padding
    // A4 aspect ratio applied to container width
      ...
      {pages.map((pg, idx) => (
        <div key={pg.id}>
          ...
          <div className="page-card" style={{ fontFamily: font, fontSize: fzMap[fontSize] }}>
            {idx === 0 ? (
              <>
                {/* Page 1: full colored header + clipped body */}
                <div className="r-header" style={{ background: accent.header }}>
              ...
              <>
                {/* Page 2+: thin accent strip + continuation */}
                ...
                <div className="page-card-cont" style={{ overflow:"hidden", maxHeight: `${pageH - 30}px` }}>
                  {/* Show overflowed content by using negative margin-top trick */}
...
// ─── Resume Page ──────────────────────────────────────────────────────────────
function ResumePage({ onBack, dark }) {
  ...
  return (
    <div className="page-wide">
      ...
      <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:"1rem" }}>
        ...
        <button className="btn-ghost" onClick={onBack} style={{ fontSize:".8rem" }}>← Rebuild</button>
      </div>
  ...
  return (
    <>
      ...
      <div className="app-shell">
        ...
        {step==="jobdesc" && <JobDescPage onBack={() => setStep("upload")} onNext={() => setStep("resume")} />}
        {step==="resume"  && <ResumePage onBack={() => setStep("jobdesc")} dark={dark} />}
      </div>
    </>
  );
}
